...
const recordMessageContainer = document.querySelector(
...
const linerNoteFormContainer = document.querySelector(
  '#liner-note-form-container'
)
const deleteLinerNoteContainer = document.querySelector(
  '#delete-liner-note-container'
)
...
const activatePlaylist = () => {
  ...
  indexRecords()
    ...
    .catch((res) => {
      if (
        res ===
        `TypeError: Cannot read properties of undefined (reading 'standoutTrack')`
      ) {
        return
      } else {
        onRecordFailure
...
export const onCreateLinerNoteSuccess = () => {
  ...
  recordMessageContainer.innerHTML = `
    ...
    `
  setTimeout(() => {
    recordMessageContainer.innerText = ``
  }, 1500)
}
